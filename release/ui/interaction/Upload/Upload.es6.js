...
function Upload(_a = {}) {
    ...
    return ({type:Flex,props:{element:'label',align:'center',justify:'center',padding:12,gap:12,wrap:true,...rest,ondragleave:handleDragLeave,ondragend:handleDragLeave,ondragover:handleDragOver,ondrop:handleDrop,style:Object.assign(Object.assign({}, style), { '--ui-upload-radius': inject(radius, radius => radius ? `${radius}px` : ''), '--ui-upload-width': inject(width, width => width ? `${width}px` : ''), '--ui-upload-height': inject(height, height => height ? `${height}px` : '') }),class:() => classes([
        ...
        ])},children:[{type:'input',props:{...props === null || props === void 0 ? void 0 : props.input,accept:accept,name:name,disabled:disabled,multiple:multiple,type:'file',ref:inputRef,'data-empty':empty,oninput:handleInput,class:() => styles.input}},{type:'div',props:{class:() => styles.labels},children:[{type:'span',props:{class:() => styles.label},children:[label]},{type:'span',props:{class:() => styles.drag},children:[{type:'slot',props:{name:'ui-upload-drag'},children:['Move the file here']}]},{type:'span',props:{class:() => styles.drop},children:[{type:'slot',props:{name:'ui-upload-drag'},children:['Drop the file here']}]}]},{type:'span',props:{class:() => styles.focus}},before,{type:'div',props:{class:() => styles.files},children:[{type:'for',props:{of:files,key:'src'},children:[(item) => {
            var _a, _b;
