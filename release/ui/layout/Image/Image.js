...
function Image(_a) {
    var { src, fallback, size, ratio = 1, radius = 8, style, loading } = _a, props = tslib.__rest(_a, ["src", "fallback", "size", "ratio", "radius", "style", "loading"]);
    ...
    return ({type:Flex.Flex,props:{...props,loading:mixLoading,style:Object.assign(Object.assign({}, style), { '--ui-image-ratio': dom.inject(ratio, ratio => Array.isArray(ratio) ? `${ratio[0]} / ${ratio[1]}` : String(ratio)), '--ui-image-radius': dom.inject(radius, radius => typeof radius === 'number' ? `${radius}px` : radius), '--ui-image-size': dom.inject(size, size => `${size}px`), 'background-image': dom.inject(safeSrc, src => `url("${src}")`) }),class:() => styles.root},children:[children]});
}
